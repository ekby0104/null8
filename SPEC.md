...
| 현재 상태 | M6 완료 — 양손 핀치 제스처 프레이밍 + 프레임 내부 이펙트 자동 순환 |
...
- 이펙트 전환: **8비트(2마디)마다 자동 순환** + 캔버스 탭 = 즉시 다음.
  이펙트 바(탭 UI)는 제거 — 현재 이펙트는 트랜스포트 FX 라벨에 표시
- 이펙트는 화면 전체가 아니라 **제스처로 설정한 프레임 내부에만** 합성된다 (M6)
- 재방문하는 이펙트는 `onReselect()`로 변형이 토글된다 (BLUEPRINT 반전 등)
...
- **이펙트 프레임**: 양손 엄지+검지 핀치로 사각형 설정 (조정 중 오렌지
...
| M6 | MediaPipe 양손 핀치 프레이밍 — 손 위치로 이펙트 프레임 설정 | 양손 핀치로 프레임 이동, 내부만 이펙트 순환 | ✅*** |
...
손 크기(0↔9)에 상대화. 양손 핀치 중에는 두 핀치 지점이 프레임의 대각
모서리가 되어 부드럽게(lerp) 따라오고, 놓으면 그 자리에 고정된다.
손 추적이 없거나 실패하면 기본 중앙 프레임으로 동작.
로드/자동 순환/합성은 헤드리스 검증 완료, 실제 핀치 제스처 반응은
실기기 확인 필요 (가짜 카메라에는 손이 없음).
