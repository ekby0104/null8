...
- 손가락 마커: 엄지·검지 끝에 각각 투명 원 + 흰 보더. 원 반지름 =
  핀치 판정 거리의 절반 → 두 원이 겹치는 순간 = 핀치 인식 시점이며
  그때 둘 다 파란 보더(#1f6bff)로 변한다
...
- **트랜스포트 바**: FX 라벨 / 프레임 카운터 / PAUSE·PLAY / PHOTO CAPTURE /
  RECORD START·STOP / REC 경과 시간(녹화 중 점멸) / FPS / Tempo / ✋ / ⇄
  (타임코드 표시는 제거 — 파일명에만 사용)
...
손 크기(0↔9)에 상대화(임계 38%), 감지 신뢰도 문턱 0.3(기본 0.5보다
완화 — 먼 거리·가장자리 손 대응), 핀치 지점이 가장자리 6% 이내면
프레임을 화면 끝까지 스냅, 양손 핀치 150ms 유지 확인 후에만
새 프레임 생성(오인식 방지), 핀치 해제 400ms 디바운스로 깜빡임 흡수.
