...
TouchDesigner 다크 테마 + **흑백(모노크롬) 컨셉**. 컬러 액센트 없이
활성 상태는 반전(흰 배경+검정 글자)과 점멸로 표현한다. **디자인 토큰:**

```
--bg:#121212  --panel:#1f1f1f  --panel2:#2a2a2a  --line:#3a3a3a
--accent:#f2f2f2 (액센트/타임라인)  --lcd:#f2f2f2 (타임코드)
폰트: SF Mono / Menlo (시스템 모노스페이스)
```

- 신호등: 그레이 스케일 3단계. 녹화 중: 버튼 반전 + 타임코드 흰색 점멸
- 프레임 테두리: 조정 중 흰 점선, 고정 흰 실선. 손 감지: ✋ 버튼 반전
