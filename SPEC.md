...
| 현재 상태 | M1 완료 — Vite+TS 구조, Canvas 2D 기반 5개 이펙트 |
| 다음 단계 | WebGL 전환 → 이펙트 추가 → 녹화 기능 |
...
│   ├── gl/
│   │   ├── context.ts       # WebGL2 컨텍스트, 텍스처 업로드 유틸 — M2
│   │   └── pipeline.ts      # 셰이더 컴파일, fullscreen quad, FBO 체인 — M2
│   └── effects/
│       ├── index.ts         # 이펙트 레지스트리 (아래 인터페이스 참조)
│       ├── raw.ts
│       ├── quadtree.ts      # CPU 계산 유지 (재귀 분할) + Canvas 2D 오버레이
│       ├── sketch.frag      # Sobel → GLSL 이식 — M2 (현재 sketch.ts)
│       ├── pointcloud.ts    # WebGL 포인트 인스턴싱 (gl.POINTS) — M3
│       ├── blueprint.frag   # Bayer dither → GLSL 이식 — M2 (현재 blueprint.ts)
...
### 5.3 SKETCH (완료 → GLSL 이식)
- Sobel 엣지 검출 → 반전 → 종이 톤 (250 기준 미색)
- v1.0: fragment shader로 이식, 풀해상도 처리 (현재는 128 저해상도라 거침)
...
### 5.5 BLUEPRINT (완료 → GLSL 이식)
- 4×4 Bayer ordered dithering + 랜덤 노이즈, 듀오톤 (흰 `#eef4fa` / 청사진 파랑 `#16489e`)
- v1.0: shader 이식, 파랑↔흰 반전 토글 (원본 1번째 컷은 파란 배경에 흰 얼굴)
