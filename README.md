...
**그 이펙트로 고정**된다(흰 테두리). 프레임을 만들 때마다 다음 이펙트가
배정되고, 프레임들은 누적되어 **여러 이펙트가 화면에 공존**한다.
새 프레임이 기존 프레임을 완전히 덮으면 기존 프레임은 제거된다.
손 추적은 시작 시 자동으로 켜진다.
...
- **트랜스포트 바**: 재생/정지, VIDEO(녹화 — mp4/webm 자동 분기), FX 라벨(다음 배정 이펙트), Tempo(BPM) 탭 변경, ✋ 손 추적 on/off, ⇄ 카메라 전환, PHOTO CAPTURE(스냅샷 저장)
