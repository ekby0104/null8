...
- **지우개 흔들기 제스처 (단계별 삭제)**: 손바닥을 펴고(네 손가락 중
  3개 이상 신전, 핀치 아님) 좌우로 1.1초 안에 3회 이상 방향 반전
  (스윙 폭 ≥ 프레임의 5%)하면 레이어를 **위에서부터 한 겹씩 삭제**:
  낙서가 있으면 낙서 먼저, 없으면 이펙트 프레임(+이펙트 순서 리셋).
  발동 시 흰색 플래시, 쿨다운 1.5초.
- 주먹 리셋 제스처는 사용자 습관과 겹쳐 제거됨. 주먹 감지는 핀치
  오인 방지(주먹 시 엄지-검지 근접)용으로만 유지.
