...
- **주먹 리셋 제스처**: **양손** 주먹(신전 손가락 ≤1)을 동시에 1초
  유지하면 **전체 리셋**(낙서 + 프레임 + 이펙트 순서). 한 손 주먹은
  일상 습관과 겹쳐 오발동이 잦아 양손 동시로만 발동. 유지 중 양손
  손바닥 위에 진행 링이 차오르고 한쪽이라도 펴면 취소. 쿨다운 2초.
  주먹 상태에서는 엄지-검지가 가까워도 핀치로 판정하지 않는다.
